...
class DecodeScriptTest(BitcoinTestFramework):
    ...
    def decodescript_script_sig(self):
        ...
        assert_equal(f"<{signature}>", rpc_result['asm'])
    ...
    def decodescript_script_pub_key(self):
        ...
        assert_equal(f"0 <{public_key_hash}>", rpc_result['segwit']['asm'])

        self.log.info("- P2PKH")
        # OP_DUP OP_HASH160 <PubKeyHash> OP_EQUALVERIFY OP_CHECKSIG
        rpc_result = self.nodes[0].decodescript('76a9' + push_public_key_hash + '88ac')
        assert_equal('pubkeyhash', rpc_result['type'])
        assert_equal(f"OP_DUP OP_HASH160 <{public_key_hash}> OP_EQUALVERIFY OP_CHECKSIG", rpc_result['asm'])
        ...
        assert_equal(f"2 <{public_key}> <{public_key}> <{public_key}> 3 OP_CHECKMULTISIG", rpc_result['asm'])
        # multisig in P2WSH
        multisig_script_hash = sha256(bytes.fromhex(multisig_script)).hex()
        assert_equal('witness_v0_scripthash', rpc_result['segwit']['type'])
        assert_equal(f"0 <{multisig_script_hash}>", rpc_result['segwit']['asm'])
        ...
        assert_equal(f"OP_IF <{public_key}> OP_CHECKSIGVERIFY OP_ELSE 500000 OP_CHECKLOCKTIMEVERIFY OP_DROP OP_ENDIF <{public_key}> OP_CHECKSIG", rpc_result['asm'])
        # CLTV script in P2WSH
        cltv_script_hash = sha256(bytes.fromhex(cltv_script)).hex()
        assert_equal(f"0 <{cltv_script_hash}>", rpc_result['segwit']['asm'])

        self.log.info("- P2PK with uncompressed pubkey")
        # <pubkey> OP_CHECKSIG
        rpc_result = self.nodes[0].decodescript(push_uncompressed_public_key + 'ac')
        assert_equal('pubkey', rpc_result['type'])
        assert_equal(f"<{uncompressed_public_key}> OP_CHECKSIG", rpc_result['asm'])
        ...
        assert_equal('multisig', rpc_result['type'])
        assert_equal(f"2 <{public_key}> <{uncompressed_public_key}> 2 OP_CHECKMULTISIG", rpc_result['asm'])
        # uncompressed pubkeys are invalid for checksigs in segwit scripts.
        # decodescript should not return a P2WPKH equivalent.
        assert 'segwit' not in rpc_result
        ...
        assert_equal(f"0 <{public_key_hash}>", rpc_result['asm'])
        ...
        assert_equal(f"0 <{p2wsh_p2pk_script_hash}>", rpc_result['asm'])
        # segwit scripts do not work nested into each other.
        # a nested segwit script should not be returned in the results.
        assert 'segwit' not in rpc_result
        ...
        assert_equal(f"1 <{xonly_public_key}>", rpc_result['asm'])
        assert 'segwit' not in rpc_result
    ...
    def decoderawtransaction_asm_sighashtype(self):
        ...
        signature_sighash_decoded = der_signature + '01' # ALL
        signature_2 = der_signature + '82'
        push_signature_2 = '48' + signature_2
        signature_2_sighash_decoded = der_signature + '82' # NONE|ANYONECANPAY

        self.log.info("- P2PK scriptSig")
        txSave.vin[0].scriptSig = bytes.fromhex(push_signature)
        rpc_result = self.nodes[0].decoderawtransaction(txSave.serialize().hex())
        assert_equal(f"<{signature_sighash_decoded}>", rpc_result['vin'][0]['scriptSig']['asm'])

        # make sure that the sighash decodes come out correctly for a more complex / lesser used case.
        txSave.vin[0].scriptSig = bytes.fromhex(push_signature_2)
        rpc_result = self.nodes[0].decoderawtransaction(txSave.serialize().hex())
        assert_equal(f"<{signature_2_sighash_decoded}>", rpc_result['vin'][0]['scriptSig']['asm'])

        self.log.info("- multisig scriptSig")
        txSave.vin[0].scriptSig = bytes.fromhex('00' + push_signature + push_signature_2)
        rpc_result = self.nodes[0].decoderawtransaction(txSave.serialize().hex())
        assert_equal(f"0 <{signature_sighash_decoded}> <{signature_2_sighash_decoded}>", rpc_result['vin'][0]['scriptSig']['asm'])
