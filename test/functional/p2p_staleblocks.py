...
"""Test stale block relay (STALEBLOCK message).

Currently implemented:
- test_basic_relay: Create stale tip, verify STALEBLOCK sent to supporting peer

Future test cases to implement:

FEATURE Negotiation:
- test_no_feature_no_relay: Peer doesn't send FEATURE -> node doesn't send STALEBLOCK
...
- test_staletips_none: Node with -staletips=none doesn't send STALEBLOCK

Receiving STALEBLOCK:
- test_receive_staleblock_new_tip: Receive STALEBLOCK -> headers processed, tip tracked, blocks requested
- test_receive_staleblock_unknown_fork_point: Unknown fork point -> message ignored
- test_receive_staleblock_too_old: Fork point too deep (>MAX_HEIGHT_DELTA) -> ignored
- test_receive_staleblock_known_tip_missing_blocks: Know tip but missing data -> request blocks
- test_receive_staleblock_relay: Receive from peer A -> relay to peer B
...
# Feature ID for stale block announcements
STALEBLOCKS_FEATURE = "https://github.com/ajtowns/bitcoin/tree/202601-staleblocks"


class StaleBlocksP2P(P2PInterface):
    """P2P interface that handles stale block messages."""

    def __init__(self):
        super().__init__()
        self.staleblocks_received = []

    def on_version(self, message):
        # Send FEATURE message to indicate we support stale blocks
        # prefer_blocks = False (we accept headers-only announcements)
        feature_data = struct.pack("<?", False)
        self.send_without_ping(msg_feature(STALEBLOCKS_FEATURE, feature_data))
        super().on_version(message)

    def on_staleblock(self, message):
        self.staleblocks_received.append(message)

    def wait_for_staleblock(self, timeout=60):
        def check():
            return len(self.staleblocks_received) > 0
        self.wait_until(check, timeout=timeout)


class StaleBlocksTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Connect P2P peer that supports stale blocks")
        peer = node.add_p2p_connection(StaleBlocksP2P())
        ...
        self.log.info("Wait for STALEBLOCK message to be sent to peer")
        peer.wait_for_staleblock()

        assert_equal(len(peer.staleblocks_received), 1)
        staleblock = peer.staleblocks_received[0]
        self.log.info(f"Received: {staleblock}")

        # Verify the message content
        assert_equal(staleblock.have_block, True)
        assert_equal(len(staleblock.headers), 1)
        ...
        assert_equal(staleblock.hash_fork_point, prev_hash)

        self.log.info("Test passed!")


if __name__ == '__main__':
    StaleBlocksTest(__file__).main()
