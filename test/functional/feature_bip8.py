...
class BIP8Test(BitcoinTestFramework):
    ...
    def setup_vbparams(self, base_height):
        ...
        self.start_nodes(extra_args = [
            ...
            # normal activation
            [f"-vbparams=testdummy:@{start}:@{stop}:0"],
            # reject blocks if not locked in by timeout
            [f"-vbparams=testdummy:@{start}:@{stop}:1"],
        ])
        for n in range(1, 4):
    ...
    def do_test(self, base_height, signalling, expstate, expheight):
        self.setup_vbparams(base_height)
        ...
        self.log.info(f"Completed test {base_height} {state}")
    ...
    def run_test(self):
        ...
        self.log.info(f"seed {SEED}, signal vals {Y}/{N}")

        self.do_test(0 * 144, [144,144,Y,0,0,0], "active", 576)
        self.do_test(6 * 144, [Y,Y,0,0,0,Y], "stopped", 612)
        self.do_test(12 * 144, [N,N,N,N,N,144], "stopped", 612)
        self.do_test(18 * 144, [0,0,0,Y,0,0], "active", 720)
        self.do_test(24 * 144, [0,144,N,N,Y,0], "active", 864)
