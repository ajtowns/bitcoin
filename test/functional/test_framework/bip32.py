...
from .key import ECKey, ECPubKey, SECP256K1, SECP256K1_ORDER, SECP256K1_G
from .script import hash256, hash160
...
class BIP32:
    ...
    def derive_one(self, i):
        ...
        if isinstance(self.key, ECPubKey):
            child.key = ECPubKey()
            child.key.valid = self.key.valid
            child.key.compressed = self.key.compressed
            child.key.p = SECP256K1.add(self.key.p, SECP256K1.mul([(SECP256K1_G, tweak)]))
        else:
            child.key = ECKey()
            child.key.secret = (self.key.secret + tweak) % SECP256K1_ORDER
