...
class WarningBitsCondition : public SignalledDeploymentStatus::Condition
{
private:
...
public:
    ...
    Consensus::SignalledDeploymentParams DeploymentParams(uint8_t bit) const { return {0, std::numeric_limits<int16_t>::max(), 0, 0, m_params.nMinerConfirmationWindow, m_params.nRuleChangeActivationThreshold, bit, false}; }

    bool operator()(const CBlockIndex* pindex, const Consensus::SignalledDeploymentParams& dep) const override
    {
        return pindex->nHeight >= m_params.MinSignalledDeploymentWarningHeight &&
...
static SignalledDeploymentStatus warningcache[Consensus::VERSIONBITS_NUM_BITS] GUARDED_BY(cs_main);
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    if (flags & SCRIPT_VERIFY_P2SH && DeploymentDisabled(consensusparams, Consensus::DEPLOYMENT_SEGWIT)) {
    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (!::ChainstateActive().IsInitialBlockDownload())
    {
        ...
        for (uint8_t bit = 0; bit < Consensus::VERSIONBITS_NUM_BITS; ++bit) {
            SignalledDeploymentStatus::State state = warningcache[bit].GetStateFor(pindex, condition.DeploymentParams(bit), condition);
            if (state == SignalledDeploymentStatus::State::ACTIVE || state == SignalledDeploymentStatus::State::LOCKED_IN) {
                const bilingual_str warning = strprintf(_("Warning: unknown new rules activated (version bit %i)"), bit);
                if (state == SignalledDeploymentStatus::State::ACTIVE) {
                    DoWarning(warning);
                } else {
...
std::vector<unsigned char> GenerateCoinbaseCommitment(CBlock& block, const CBlockIndex* pindexPrev, const Consensus::Params& consensusParams)
{
    ...
    if (!DeploymentDisabled(consensusParams, Consensus::DEPLOYMENT_SEGWIT)) {
        if (commitpos == -1) {
