...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    if (VersionBitsActive(pindex->pprev, consensusparams, Consensus::DEPLOYMENT_STRICTDER, versionbitscache)) {
        flags |= SCRIPT_VERIFY_DERSIG;
    }

    // Start enforcing CHECKLOCKTIMEVERIFY (BIP65) rule
    if (VersionBitsActive(pindex->pprev, consensusparams, Consensus::DEPLOYMENT_CLTV, versionbitscache)) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if((block.nVersion < 2 && nHeight >= consensusParams.BIP34Height) ||
       (block.nVersion < 3 && VersionBitsActive(pindexPrev, consensusParams, Consensus::DEPLOYMENT_STRICTDER, versionbitscache)) ||
       (block.nVersion < 4 && VersionBitsActive(pindexPrev, consensusParams, Consensus::DEPLOYMENT_CLTV, versionbitscache))) {
