...
inline void SetDeployment(Consensus::Params& params, Consensus::SignalledDeployment pos, const ArgsManager& args, bool always_guaranteed=false)
{
    ...
    params.vDeployments[(int)pos] = Consensus::Deployment<bit,start_height,primary_periods,quiet_periods,secondary_periods,period,threshold>(guaranteed);
}
...
public:
    explicit CMainParams(const ArgsManager& args) {
        ...
        consensus.MinSignalledDeploymentWarningHeight = 483840; // segwit activation height + miner confirmation window
...
public:
    explicit CTestNetParams(const ArgsManager& args) {
        ...
        consensus.MinSignalledDeploymentWarningHeight = 836640; // segwit activation height + miner confirmation window
        consensus.powLimit = uint256S("00000000ffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
        consensus.nPowTargetTimespan = 14 * 24 * 60 * 60; // two weeks
        consensus.nPowTargetSpacing = 10 * 60;
...
public:
    explicit CRegTestParams(const ArgsManager& args) {
        ...
        consensus.MinSignalledDeploymentWarningHeight = 0;
     ...
     */
    void UpdateVersionBitsParameters(Consensus::SignalledDeployment d, int start_height, int16_t primary_periods)
    {
    ...
    for (const std::string& strDeployment : args.GetArgs("-vbparams")) {
        ...
        for (int j=0; j < (int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS; ++j) {
            if (vDeploymentParams[0] == Consensus::DeploymentInfo[j].name) {
                UpdateVersionBitsParameters(Consensus::SignalledDeployment(j), start_height, primary_periods);
                found = true;
