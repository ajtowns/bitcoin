...
static std::string gbt_vb_name(const Consensus::SignalledDeployment pos) {
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    for (int j = 0; j < (int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS; ++j) {
        Consensus::SignalledDeployment pos = Consensus::SignalledDeployment(j);
        SignalledDeploymentStatus::State state = g_deploymentstatus.GetStateFor(pindexPrev, consensusParams, pos);
        switch (state) {
            case SignalledDeploymentStatus::State::DEFINED:
            case SignalledDeploymentStatus::State::FAILED:
                // Not exposed to GBT at all
                break;
            case SignalledDeploymentStatus::State::LOCKED_IN:
            ...
            case SignalledDeploymentStatus::State::ACTIVE:
            {
                // Add to rules only
                const auto& info = Consensus::DeploymentInfo[pos];
