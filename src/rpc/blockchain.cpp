...
static void SignalledDeploymentDescPushBack(UniValue& softforks, const std::string &name, const Consensus::Params& consensusParams, Consensus::SignalledDeployment id) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    if (DeploymentDisabled(consensusParams, id)) return;

    UniValue rv(UniValue::VOBJ);
    rv.pushKV("type", "modern");

    const CBlockIndex* tip = ::ChainActive().Tip();
    const Consensus::SignalledDeploymentParams& dep = consensusParams.vDeployments[id];

    // For modern deployments.
    UniValue depdesc(UniValue::VOBJ);
    const SignalledDeploymentStatus::StateHeight stateheight = g_deploymentstatus.GetStateHeightFor(tip, consensusParams, id);

    bool signal = false;
    switch (stateheight.state) {
    case SignalledDeploymentStatus::State::DEFINED: depdesc.pushKV("status", "defined"); break;
    case SignalledDeploymentStatus::State::PRIMARY:
        depdesc.pushKV("status", "primary");
        signal = true;
        break;
    case SignalledDeploymentStatus::State::QUIET:
        depdesc.pushKV("status", "quiet");
        signal = true;
        break;
    case SignalledDeploymentStatus::State::SECONDARY:
        depdesc.pushKV("status", "secondary");
        signal = true;
        break;
    case SignalledDeploymentStatus::State::LOCKED_IN: depdesc.pushKV("status", "locked_in"); break;
    case SignalledDeploymentStatus::State::ACTIVE: depdesc.pushKV("status", "active"); break;
    case SignalledDeploymentStatus::State::FAILED: depdesc.pushKV("status", "failed"); break;
    }
    ...
    if (signal) {
        ...
        SignalledDeploymentStatus::Stats statsStruct = g_deploymentstatus.GetStateStatisticsFor(tip, consensusParams, id);
    ...
    if (SignalledDeploymentStatus::State::ACTIVE == stateheight.state) {
        rv.pushKV("height", stateheight.height);
    } else if (SignalledDeploymentStatus::State::LOCKED_IN == stateheight.state) {
        rv.pushKV("height", stateheight.height + dep.period);
    }
    rv.pushKV("active", SignalledDeploymentStatus::State::ACTIVE == stateheight.state);
...
UniValue getblockchaininfo(const JSONRPCRequest& request)
{
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::OBJ_DYN, "softforks", "status of softforks",
                        {
                            ...
                            {
                                {RPCResult::Type::STR, "type", "one of \"buried\", \"modern\""},
                                {RPCResult::Type::OBJ, "modern", "status of bip9 softforks (only for \"bip9\" type)",
                                {
                                    ...
                                    {RPCResult::Type::OBJ, "statistics", "numeric statistics about signalling for a softfork (only for \"started\" status)",
                                    {
    ...
    for (int i = 0; i < Consensus::MAX_VERSION_BITS_DEPLOYMENTS; ++i) {
        SignalledDeploymentDescPushBack(softforks, Consensus::DeploymentInfo[i].name, consensusParams, (Consensus::SignalledDeployment)i);
    }
