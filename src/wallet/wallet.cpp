...
void CWallet::SetMinVersion(enum WalletFeature nVersion, WalletBatch* batch_in)
{
    ...
    if (nWalletVersion >= nVersion)
        return;
    WalletLogInfo("Setting minversion to %d\n", nVersion);
...
bool CWallet::SubmitTxMemoryPoolAndRelay(CWalletTx& wtx, std::string& err_string, bool relay) const
{
    ...
    WalletLogInfo("Submitting wtx %s to mempool for relay\n", wtx.GetHash().ToString());
...
bool CWallet::UpgradeWallet(int version, bilingual_str& error)
{
    ...
    if (version == 0) {
        WalletLogInfo("Performing wallet upgrade to %i\n", FEATURE_LATEST);
        version = FEATURE_LATEST;
    } else {
        WalletLogInfo("Allowing wallet upgrade up to %i\n", version);
    }
...
ScriptPubKeyMan* CWallet::AddWalletDescriptor(WalletDescriptor& desc, const FlatSigningProvider& signing_provider, const std::string& label, bool internal)
{
    ...
    if (!IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS)) {
        WalletLogInfo("Cannot add WalletDescriptor to a non-descriptor wallet\n");
        return nullptr;
    }
    ...
    if (!spk_man->TopUp()) {
        WalletLogInfo("Could not top up scriptPubKeys\n");
        return nullptr;
    }
    ...
    if (!desc.descriptor->IsRange()) {
        ...
        if (script_pub_keys.empty()) {
            WalletLogInfo("Could not generate scriptPubKeys (cache is empty)\n");
            return nullptr;
        }
...
bool CWallet::ApplyMigrationData(MigrationData& data, bilingual_str& error)
{
    ...
    WalletLogInfo("Wallet migration complete.\n");
