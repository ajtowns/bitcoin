...
bool CheckTransaction(const CTransaction& tx, TxValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (tx.vin.empty())
        return state.Invalid(TxValidationResult::CONSENSUS, false, REJECT_INVALID, "bad-txns-vin-empty");
    if (tx.vout.empty())
        return state.Invalid(TxValidationResult::CONSENSUS, false, REJECT_INVALID, "bad-txns-vout-empty");
    // Size limits (this doesn't take the witness into account, as that hasn't been checked for malleability)
    if (::GetSerializeSize(tx, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) * WITNESS_SCALE_FACTOR > MAX_BLOCK_WEIGHT)
        return state.Invalid(TxValidationResult::CONSENSUS, false, REJECT_INVALID, "bad-txns-oversize");
