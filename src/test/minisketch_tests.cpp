...
BOOST_AUTO_TEST_CASE(minisketch46_test)
{
    for (int i = 0; i < 100; ++i) {
        uint32_t errors = 0 + m_rng.randrange(11);
        uint64_t start_a = 1 + m_rng.randrange<uint64_t>(1ULL << 44);
        uint32_t a_not_b = m_rng.randrange(errors + 1);
        uint32_t b_not_a = errors - a_not_b;
        uint32_t both = m_rng.randrange(10000);
        ...
        Minisketch sketch_a = MakeMinisketch46(10);
        for (uint64_t a = start_a; a < end_a; ++a) sketch_a.Add(a);
        Minisketch sketch_b = MakeMinisketch46(10);
        for (uint64_t b = start_b; b < end_b; ++b) sketch_b.Add(b);

        Minisketch sketch_ar = MakeMinisketch46(10);
        Minisketch sketch_br = MakeMinisketch46(10);
        sketch_ar.Deserialize(sketch_a.Serialize());
        sketch_br.Deserialize(sketch_b.Serialize());
