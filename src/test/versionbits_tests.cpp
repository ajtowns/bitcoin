...
static void check_computeblockversion(ChainstateManager& chainman, Consensus::DeploymentPos dep)
{
    const Consensus::Params& params = chainman.GetConsensus();

    // This implicitly uses g_versionbitscache, so clear it every time
    chainman.m_versionbitscache.Clear();
    ...
    BOOST_CHECK_EQUAL(chainman.m_versionbitscache.ComputeBlockVersion(nullptr, params), VERSIONBITS_TOP_BITS);
    ...
    const uint32_t bitmask{chainman.m_versionbitscache.Mask(params, dep)};
    ...
    if (nTime == 0) {
        ...
        BOOST_CHECK_EQUAL(chainman.m_versionbitscache.ComputeBlockVersion(lastBlock, params) & (1 << bit), 0);
        lastBlock = firstChain.Mine(params.nMinerConfirmationWindow, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK((chainman.m_versionbitscache.ComputeBlockVersion(lastBlock, params) & (1 << bit)) != 0);
    ...
    BOOST_CHECK_EQUAL(chainman.m_versionbitscache.ComputeBlockVersion(lastBlock, params) & VERSIONBITS_TOP_MASK, VERSIONBITS_TOP_BITS);
...
BOOST_AUTO_TEST_CASE(versionbits_computeblockversion)
{
    ...
    for (const auto& chain_name : {CBaseChainParams::MAIN, CBaseChainParams::TESTNET, CBaseChainParams::SIGNET, CBaseChainParams::REGTEST}) {
        ...
        for (int i = 0; i < (int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS; ++i) {
            ...
            // times are picked.
            const uint32_t dep_mask{m_node.chainman->m_versionbitscache.Mask(chainParams->GetConsensus(), dep)};
            BOOST_CHECK(!(chain_all_vbits & dep_mask));
            chain_all_vbits |= dep_mask;
            check_computeblockversion(*m_node.chainman, dep);
        }
    }

    {
        ...
        check_computeblockversion(*m_node.chainman, Consensus::DEPLOYMENT_TESTDUMMY);
    }
