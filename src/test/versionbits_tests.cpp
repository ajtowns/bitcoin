...
static void check_computeblockversion(VersionBitsCache& versionbitscache, const Consensus::Params& params, Consensus::DeploymentPos dep)
{
    ...
    BOOST_REQUIRE(threshold < window); // must be able to have a window that does activate
    ...
    BOOST_REQUIRE_EQUAL(min_activation_height % window, 0U);
    ...
    if (nTime == 0) {
        ...
        lastBlock = firstChain.Mine(window - 1, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK_EQUAL(versionbitscache.ComputeBlockVersion(lastBlock, params) & (1 << bit), 0);
        lastBlock = firstChain.Mine(window, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        ...
        for (uint32_t i = 1; i < window - 4; i++) {
            lastBlock = firstChain.Mine(window + i, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
            BOOST_CHECK_EQUAL(versionbitscache.ComputeBlockVersion(lastBlock, params) & (1 << bit), 0);
        }
        // Now mine 5 more blocks at the start time -- MTP should not have passed yet, so
        // CBV should still not yet set the bit.
        nTime = nStartTime;
        for (uint32_t i = window - 4; i <= window; i++) {
            lastBlock = firstChain.Mine(window + i, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
            BOOST_CHECK_EQUAL(versionbitscache.ComputeBlockVersion(lastBlock, params) & (1 << bit), 0);
        }
    ...
    lastBlock = firstChain.Mine(window * 3, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    ...
    nTime += 600;
    uint32_t blocksToMine = window * 2; // test blocks for up to 2 time periods
    uint32_t nHeight = window * 3;
    ...
    // next period.
    lastBlock = secondChain.Mine(window, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK((versionbitscache.ComputeBlockVersion(lastBlock, params) & (1 << bit)) != 0);

    // Mine another period worth of blocks, signaling the new bit.
    lastBlock = secondChain.Mine(window * 2, nTime, VERSIONBITS_TOP_BITS | (1<<bit)).Tip();
    ...
    lastBlock = secondChain.Mine((window * 3) - 1, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK((versionbitscache.ComputeBlockVersion(lastBlock, params) & (1 << bit)) != 0);
    lastBlock = secondChain.Mine(window * 3, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
