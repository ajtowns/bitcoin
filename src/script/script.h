...
public:
    ...
    /** Check whether a byte vector is the minimal encoding of a script number.
     *
     * The most significant byte - excluding the sign bit - must be non-zero,
     * otherwise the number could be encoded in fewer bytes. This also rejects
     * the negative-zero encoding, 0x80.
     *
     * One exception: if there's more than one byte and the most significant
     * bit of the second-most-significant byte is set, it would conflict with
     * the sign bit. An example of this case is +-255, which encode to 0xff00
     * and 0xff80 respectively (big-endian).
     */
    static bool CheckMinimalNumber(std::span<const unsigned char> vch)
    {
        if (vch.size() > 0 && (vch.back() & 0x7f) == 0) {
