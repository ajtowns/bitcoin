...
public:
    ...
    std::vector<CAddress> GetAddresses(size_t max_addresses, size_t max_pct, std::optional<Network> network) override;
...
private:
    ...
    */
    ...
    void PushMessage(CNode* pnode, CSerializedNetMsg&& msg) const
    {
        pnode->PushMessage(std::move(msg), m_opts.send_buffer_max_size);
    }
    ...
    m_connman.ForNode(nodeid, [this](CNode* pfrom) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
        ...
        if (lNodesAnnouncingHeaderAndIDs.size() >= 3) {
            ...
            m_connman.ForNode(lNodesAnnouncingHeaderAndIDs.front(), [this](CNode* pnodeStop){
                PushMessage(pnodeStop, CNetMsgMaker(pnodeStop->GetCommonVersion()).Make(NetMsgType::SENDCMPCT, /*high_bandwidth=*/false, /*version=*/CMPCTBLOCKS_VERSION));
        ...
        PushMessage(pfrom, CNetMsgMaker(pfrom->GetCommonVersion()).Make(NetMsgType::SENDCMPCT, /*high_bandwidth=*/true, /*version=*/CMPCTBLOCKS_VERSION));
...
void PeerManagerImpl::PushNodeVersion(CNode& pnode, const Peer& peer)
{
    ...
    PushMessage(&pnode, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::VERSION, PROTOCOL_VERSION, my_services, nTime,
...
std::optional<std::string> PeerManagerImpl::FetchBlock(NodeId peer_id, const CBlockIndex& block_index)
{
    ...
    bool success = m_connman.ForNode(peer_id, [this, &invs](CNode* node) {
        ...
        this->PushMessage(node, msgMaker.Make(NetMsgType::GETDATA, invs));
        return true;
    });
...
void PeerManagerImpl::ProcessGetBlockData(CNode& pfrom, Peer& peer, const CInv& inv)
{
    ...
    if (a_recent_block && a_recent_block->GetHash() == pindex->GetBlockHash()) {
        ...
        PushMessage(&pfrom, msgMaker.Make(NetMsgType::BLOCK, Span{block_data}));
    ...
    if (pblock) {
        if (inv.IsMsgBlk()) {
            PushMessage(&pfrom, msgMaker.Make(SERIALIZE_TRANSACTION_NO_WITNESS, NetMsgType::BLOCK, *pblock));
        } else if (inv.IsMsgWitnessBlk()) {
            PushMessage(&pfrom, msgMaker.Make(NetMsgType::BLOCK, *pblock));
            ...
            if (sendMerkleBlock) {
                PushMessage(&pfrom, msgMaker.Make(NetMsgType::MERKLEBLOCK, merkleBlock));
                ...
                for (PairType& pair : merkleBlock.vMatchedTxn)
                    PushMessage(&pfrom, msgMaker.Make(SERIALIZE_TRANSACTION_NO_WITNESS, NetMsgType::TX, *pblock->vtx[pair.first]));
            }
            // else
            // no response
            ...
            if (CanDirectFetch() && pindex->nHeight >= m_chainman.ActiveChain().Height() - MAX_CMPCTBLOCK_DEPTH) {
                if (a_recent_compact_block && a_recent_compact_block->header.GetHash() == pindex->GetBlockHash()) {
                    PushMessage(&pfrom, msgMaker.Make(NetMsgType::CMPCTBLOCK, *a_recent_compact_block));
                } else {
                    CBlockHeaderAndShortTxIDs cmpctblock{*pblock};
                    PushMessage(&pfrom, msgMaker.Make(NetMsgType::CMPCTBLOCK, cmpctblock));
                }
            } else {
                PushMessage(&pfrom, msgMaker.Make(NetMsgType::BLOCK, *pblock));
            }
...

    {
        ...
        if (inv.hash == peer.m_continuation_block) {
            ...
            PushMessage(&pfrom, msgMaker.Make(NetMsgType::INV, vInv));
...
void PeerManagerImpl::ProcessGetData(CNode& pfrom, Peer& peer, const std::atomic<bool>& interruptMsgProc)
{
    ...
    while (it != peer.m_getdata_requests.end() && it->IsGenTxMsg()) {
        ...
        if (pfrom.IsSendingPaused()) break;
        ...
        if (tx) {
            ...
            PushMessage(&pfrom, msgMaker.Make(nSendFlags, NetMsgType::TX, *tx));
    ...
    if (it != peer.m_getdata_requests.end() && !pfrom.IsSendingPaused()) {
        const CInv &inv = *it++;
        if (inv.IsGenBlkMsg()) {
    ...
    if (!vNotFound.empty()) {
        ...
        PushMessage(&pfrom, msgMaker.Make(NetMsgType::NOTFOUND, vNotFound));
    }
}
...
void PeerManagerImpl::SendBlockTransactions(CNode& pfrom, Peer& peer, const CBlock& block, const BlockTransactionsRequest& req)
{
    ...
    const CNetMsgMaker msgMaker(pfrom.GetCommonVersion());
    PushMessage(&pfrom, msgMaker.Make(NetMsgType::BLOCKTXN, resp));
}
...
bool PeerManagerImpl::MaybeSendGetHeaders(CNode& pfrom, const CBlockLocator& locator, Peer& peer)
{
    ...
    if (current_time - peer.m_last_getheaders_timestamp > HEADERS_RESPONSE_TIME) {
        PushMessage(&pfrom, msgMaker.Make(NetMsgType::GETHEADERS, locator, uint256()));
    ...
    if (CanDirectFetch() && last_header.IsValid(BLOCK_VALID_TREE) && m_chainman.ActiveChain().Tip()->nChainWork <= last_header.nChainWork) {
        ...
        if (!m_chainman.ActiveChain().Contains(pindexWalk)) {
            ...
            if (vGetData.size() > 0) {
                ...
                PushMessage(&pfrom, msgMaker.Make(NetMsgType::GETDATA, vGetData));
            }
        }
    }
}
    ...
    const CNetMsgMaker msgMaker(pfrom.GetCommonVersion());
    {
        ...
        if (status == READ_STATUS_INVALID) {
            ...
            if (first_in_flight) {
                ...
                PushMessage(&pfrom, msgMaker.Make(NetMsgType::GETDATA, invs));
            } else {
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        if (greatest_common_version >= WTXID_RELAY_VERSION) {
            PushMessage(&pfrom, msg_maker.Make(NetMsgType::WTXIDRELAY));
        }
        ...
        if (greatest_common_version >= 70016) {
            ...
            PushMessage(&pfrom, msg_maker.Make(NetMsgType::SENDADDRV2));
        }
        ...
        if (greatest_common_version >= WTXID_RELAY_VERSION && m_txreconciliation) {
            ...
            if (tx_relay && WITH_LOCK(tx_relay->m_bloom_filter_mutex, return tx_relay->m_relay_txs) &&
                ...
                PushMessage(&pfrom, msg_maker.Make(NetMsgType::SENDTXRCNCL,
                                                             TXRECONCILIATION_VERSION, recon_salt));
            }
        }

        PushMessage(&pfrom, msg_maker.Make(NetMsgType::VERACK));
        ...
        if (send_getaddr) {
            ...
            PushMessage(&pfrom, CNetMsgMaker(greatest_common_version).Make(NetMsgType::GETADDR));
        ...
        if (greatest_common_version <= 70012) {
            ...
            PushMessage(&pfrom, CNetMsgMaker(greatest_common_version).Make("alert", Span{finalAlert}));
        }
    ...
    if (msg_type == NetMsgType::VERACK) {
        ...
        if (pfrom.GetCommonVersion() >= SHORT_IDS_BLOCKS_VERSION) {
            ...
            PushMessage(&pfrom, msgMaker.Make(NetMsgType::SENDCMPCT, /*high_bandwidth=*/false, /*version=*/CMPCTBLOCKS_VERSION));
        }
    ...
    if (msg_type == NetMsgType::GETHEADERS) {
        ...
        if (m_chainman.ActiveTip() == nullptr ||
            ...
            PushMessage(&pfrom, msgMaker.Make(NetMsgType::HEADERS, std::vector<CBlock>()));
            return;
        }
        ...
        PushMessage(&pfrom, msgMaker.Make(NetMsgType::HEADERS, vHeaders));
        return;
    }
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (pindex->nChainWork <= m_chainman.ActiveChain().Tip()->nChainWork || // We know something better
            ...
            if (requested_block_from_this_peer) {
                ...
                PushMessage(&pfrom, msgMaker.Make(NetMsgType::GETDATA, vInv));
            }
            return;
        }
        ...
        if (pindex->nHeight <= m_chainman.ActiveChain().Height() + 2) {
            if ((already_in_flight < MAX_CMPCTBLOCKS_INFLIGHT_PER_BLOCK && nodestate->vBlocksInFlight.size() < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                if (req.indexes.empty()) {
                    ...
                    PushMessage(&pfrom, msgMaker.Make(NetMsgType::GETBLOCKTXN, req));
    ...
    if (msg_type == NetMsgType::GETADDR) {
        ...
        if (pfrom.HasPermission(NetPermissionFlags::Addr)) {
            vAddr = GetAddresses(MAX_ADDR_TO_SEND, MAX_PCT_ADDR_TO_SEND, /*network=*/std::nullopt);
        } else {
            vAddr = GetAddresses(pfrom, MAX_ADDR_TO_SEND, MAX_PCT_ADDR_TO_SEND);
        }
        FastRandomContext insecure_rand;
    ...
    if (msg_type == NetMsgType::PING) {
        if (pfrom.GetCommonVersion() > BIP0031_VERSION) {
            ...
            // return very quickly.
            PushMessage(&pfrom, msgMaker.Make(NetMsgType::PONG, nonce));
        }
        return;
    }
...
bool PeerManagerImpl::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (pfrom->IsSendingPaused()) return false;
...
void PeerManagerImpl::MaybeSendPing(CNode& node_to, Peer& peer, std::chrono::microseconds now)
{
    ...
    if (pingSend) {
        ...
        if (node_to.GetCommonVersion() > BIP0031_VERSION) {
            ...
            PushMessage(&node_to, msgMaker.Make(NetMsgType::PING, nonce));
        } else {
            // Peer is too old to support ping command with nonce, pong will never arrive.
            peer.m_ping_nonce_sent = 0;
            PushMessage(&node_to, msgMaker.Make(NetMsgType::PING));
        }
    }
}
...
void PeerManagerImpl::MaybeSendAddr(CNode& node, Peer& peer, std::chrono::microseconds current_time)
{
    ...
    PushMessage(&node, CNetMsgMaker(node.GetCommonVersion()).Make(make_flags, msg_type, peer.m_addrs_to_send));
    peer.m_addrs_to_send.clear();
...
void PeerManagerImpl::MaybeSendSendHeaders(CNode& node, Peer& peer)
{
    ...
    if (!peer.m_sent_sendheaders && node.GetCommonVersion() >= SENDHEADERS_VERSION) {
        ...
        if (state.pindexBestKnownBlock != nullptr &&
            ...
            // nodes)
            PushMessage(&node, CNetMsgMaker(node.GetCommonVersion()).Make(NetMsgType::SENDHEADERS));
...
void PeerManagerImpl::MaybeSendFeefilter(CNode& pto, Peer& peer, std::chrono::microseconds current_time)
{
    ...
    if (current_time > peer.m_next_send_feefilter) {
        ...
        if (filterToSend != peer.m_fee_filter_sent) {
            PushMessage(&pto, CNetMsgMaker(pto.GetCommonVersion()).Make(NetMsgType::FEEFILTER, filterToSend));
...

    {
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.m_requested_hb_cmpctblocks) {
                    ...
                    if (cached_cmpctblock_msg.has_value()) {
                        ...
                        PushMessage(pto, msgMaker.Make(NetMsgType::CMPCTBLOCK, cmpctblock));
                    }
                    ...
                    PushMessage(pto, msgMaker.Make(NetMsgType::HEADERS, vHeaders));
                    state.pindexBestHeaderSent = pBestIndex;
                } else
                    fRevertToInv = true;
            }
        ...
        std::vector<CInv> vInv;
        {
            ...
            for (const uint256& hash : peer->m_blocks_for_inv_relay) {
                ...
                if (vInv.size() == MAX_INV_SZ) {
                    PushMessage(pto, msgMaker.Make(NetMsgType::INV, vInv));
                    vInv.clear();
                }
            }
        ...
        for (const GenTxid& gtxid : requestable) {
            if (!AlreadyHaveTx(gtxid)) {
                ...
                if (vGetData.size() >= MAX_GETDATA_SZ) {
                    PushMessage(pto, msgMaker.Make(NetMsgType::GETDATA, vGetData));
                    vGetData.clear();
                }
...
std::vector<CAddress> PeerManagerImpl::GetAddresses(size_t max_addresses, size_t max_pct, std::optional<Network> network)
{
    std::vector<CAddress> addresses = m_addrman.GetAddr(max_addresses, max_pct, network);
    if (m_banman) {
...
std::vector<CAddress> PeerManagerImpl::GetAddresses(CNode& requestor, size_t max_addresses, size_t max_pct)
{
    auto local_socket_bytes = requestor.addrBind.GetAddrBytes();
    uint64_t cache_id = m_connman.GetDeterministicRandomizer(RANDOMIZER_ID_ADDRCACHE)
