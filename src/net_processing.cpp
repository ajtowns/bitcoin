...
public:
    ...
    bool SendMessages(CNode& node) override
        EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex, !m_most_recent_block_mutex, g_msgproc_mutex, !m_tx_download_mutex, !m_inv_to_send_mutex, !m_template_mutex);
...
private:
    ...
    */
    ...
    void ProcessTemplateSketchUpdate(CNode& node, Peer& peer, int round, uint8_t basis_id, const node::TemplateManager::TmpltResult& result)
        EXCLUSIVE_LOCKS_REQUIRED(g_msgproc_mutex, m_template_mutex);
    ...
    /** Wrap InitData with the template pool as an extra tx source. */
    ReadStatus InitCompactBlockData(PartiallyDownloadedBlock& partial_block,
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::GETTMPLT) {
        ...
        if (round == 0) {
            ...
            // round=1..4: request for more reconstruction data for previously sketched template
            uint256 hash;
            uint8_t basis_id;
            node::GroupMask shortidmask, sketchmask;
            uint16_t raw_sketchmask{0};
            uint32_t raw_shortidmask{0};
            vRecv >> hash >> basis_id >> raw_sketchmask >> raw_shortidmask;
            if (round == 4 && raw_sketchmask != 0) {
                LogDebug(BCLog::GETTMPLT, "Invalid non-empty sketchmask at round 4, disconnecting %s", pfrom.LogPeer());
                pfrom.fDisconnect = true;
                return;
            }
            sketchmask.FromUints<uint16_t>(std::span{&raw_sketchmask, 1});
            ...
            if (shortidmask.Any()) shortid_bytes = tmpl->GetShortIDBytes(round, basis_id, shortidmask);
            ...
            if (sketchmask.Any()) {
                ...
                for (int i : sketchmask) {
                    sketches_to_send.push_back(all_sketches[i]);
                }
            }

            uint32_t shortidmask_raw{0};
    ...
    if (msg_type == NetMsgType::GETTMPLTTXN) {
        ...
        node::RequestedTemplateTxns queued;
        if (!queued.Queue(hash, *tmpl, grenc)) {
            LogDebug(BCLog::GETTMPLT, "Disconnecting: invalid gettmplttxn positions peer=%d", pfrom.GetId());
            pfrom.fDisconnect = true;
            return;
        }
    ...
    if (msg_type == NetMsgType::TMPLT) {
        ...
        if (round == 0) {
            ...
            node::GRVector shortid_bytes;
            vRecv >> tip_hash >> nonce >> basis_hash >> basis_id >> basis_delta >> LIMITED_VECTOR(sketches, 4);
            if (!vRecv.empty()) {
                vRecv >> shortidmask_raw >> shortid_bytes;
                if (shortid_bytes.n_elements > 300) {
                    LogDebug(BCLog::GETTMPLT, "Got tmplt round=0 with too many shortids (%d > 300), disconnecting peer=%d",
                             shortid_bytes.n_elements, pfrom.GetId());
                    pfrom.fDisconnect = true;
                    return;
                }
            }
            shortidmask.FromUints<uint8_t>(std::span{&shortidmask_raw, 1});
            ...
            node::GRVector shortid_bytes;
            std::vector<node::LocalTemplate::Sketch> sketches;
            vRecv >> basis_id >> sketchmask_raw >> LIMITED_VECTOR(sketches, 16) >> shortidmask_raw >> shortid_bytes;
            ...
            auto result = m_templateman.UpdatePeerSketch(pfrom.GetId(), hash, round,
                                                         ...
                                                         NodeClock::now());
            ProcessTemplateSketchUpdate(pfrom, peer, round, basis_id, result);
        } else {
...
void PeerManagerImpl::MaybeGenerateTemplate()
{
    ...
    if (*first_template) {
        ...
        if (!m_mempool.GetLoadTried()) {
            LogDebug(BCLog::GETTMPLT, "Skipping template generation (mempool not loaded)");
            return;
        }
        if (WITH_LOCK(cs_main, return !CanDirectFetch())) {
            LogDebug(BCLog::GETTMPLT, "Skipping template generation (tip too old)");
            return;
        }
    }

    node::BlockCreateOptions opt;
    opt.block_min_fee_rate = CFeeRate(0);
...
void PeerManagerImpl::ProcessTemplateSketchUpdate(CNode& node, Peer& peer, int round, uint8_t basis_id, const node::TemplateManager::TmpltResult& result)
{
    AssertLockHeld(g_msgproc_mutex);
    ...
    case Unresolved:
    {
        ...
        uint16_t sketchmask_raw{0};
        uint32_t shortidmask_raw{0};
        result.sketchmask.ToUints(std::span{&sketchmask_raw, 1});
        result.shortidmask.ToUints(std::span{&shortidmask_raw, 1});
        LogDebug(BCLog::GETTMPLT, "Sending gettmplt round=%d template=%s basis_id=%d shortidmask=0x%08x sketchmask=0x%08x peer=%d",
                 round + 1, result.hash.ToString(), basis_id, shortidmask_raw, sketchmask_raw, node.GetId());
        MakeAndPushMessage(node, NetMsgType::GETTMPLT, static_cast<uint8_t>(round + 1),
                           result.hash, basis_id, sketchmask_raw, shortidmask_raw);
        return;
    }
    case NeedsTxs:
    {
...
void PeerManagerImpl::MaybeRequestTemplate(CNode& node, Peer& peer)
{
    ...
    if (basis_hash.IsNull()) {
        MakeAndPushMessage(node, NetMsgType::GETTMPLT, uint8_t{0});
    } else {
...
void PeerManagerImpl::MaybeSendTemplateMessages(CNode& node, Peer& peer)
{
    ...
    if (auto* txns = std::get_if<node::RequestedTemplateTxns>(&peer.m_tmplt_request); txns) {
        ...
        if (tmpl) {
            ...
            if (!txs.empty()) {
                MakeAndPushMessage(node, NetMsgType::TMPLTTXN, tmpl->m_hash, TX_WITH_WITNESS(txs));
            }
            if (txns->empty()) peer.m_tmplt_request = std::monostate{};
        } else {
            peer.m_tmplt_request = std::monostate{};
        }
    } else if (auto* req = std::get_if<node::TemplateManager::Req>(&peer.m_tmplt_request); req) {
        // Send pending tmplt n=0 response if we have a template available.
        LOCK(m_template_mutex);
        auto ltd = m_templateman.GetRequestedTemplate(*req);
        if (!ltd.tmpl) return; // no generated template ready

        // Ready to respond, so clear the request.
        peer.m_tmplt_request = std::monostate{};
...
namespace {
/** ExtraTransactions that iterates template pool txns then vExtraTxnForCompact. */
...
public:
    ...
    WitRef next() override
    {
        ...
        while (m_extra_idx < m_extra.size()) {
            ...
            if (item.second != nullptr) return {&item.first, &item.second};
        }
...
ReadStatus PeerManagerImpl::InitCompactBlockData(PartiallyDownloadedBlock& partial_block,
                                                  const CBlockHeaderAndShortTxIDs& cmpctblock)
{
    AssertLockHeld(g_msgproc_mutex);
    AssertLockNotHeld(m_template_mutex);
    LOCK(m_template_mutex);
    return partial_block.InitData(cmpctblock, m_mempool,
                                  CombinedExtraTransactions(vExtraTxnForCompact,
