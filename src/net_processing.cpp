...
void PeerManagerImpl::FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller)
{
    ...
    while (pindexWalk->nHeight < nMaxHeight) {
        ...
        for (const CBlockIndex* pindex : vToFetch) {
            ...
            if (!State(nodeid)->fHaveWitness && DeploymentActiveAt<Consensus::DEPLOYMENT_SEGWIT>(*pindex, m_chainman)) {
...
void PeerManagerImpl::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock)
{
    ...
    bool fWitnessEnabled = DeploymentActiveAt<Consensus::DEPLOYMENT_SEGWIT>(*pindex, m_chainman);
        ...
        if (CanDirectFetch() && pindexLast->IsValid(BLOCK_VALID_TREE) && m_chainman.ActiveChain().Tip()->nChainWork <= pindexLast->nChainWork) {
            ...
            while (pindexWalk && !m_chainman.ActiveChain().Contains(pindexWalk) && vToFetch.size() <= MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
                if (!(pindexWalk->nStatus & BLOCK_HAVE_DATA) &&
                        ...
                        (!DeploymentActiveAt<Consensus::DEPLOYMENT_SEGWIT>(*pindexWalk, m_chainman) || State(pfrom.GetId())->fHaveWitness)) {
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (DeploymentActiveAt<Consensus::DEPLOYMENT_SEGWIT>(*pindex, m_chainman) && !nodestate->fSupportsDesiredCmpctVersion) {
