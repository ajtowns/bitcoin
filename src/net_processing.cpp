...
static constexpr std::chrono::seconds ORPHAN_TX_EXPIRE_TIME{20 * 60};
/** Minimum time between orphan transactions expire time checks in seconds */
static constexpr std::chrono::seconds ORPHAN_TX_EXPIRE_INTERVAL{5 * 60};
...
static constexpr std::chrono::seconds AVG_FEEFILTER_BROADCAST_INTERVAL{10 * 60};
/** Maximum feefilter broadcast delay after significant change. */
static constexpr std::chrono::seconds MAX_FEEFILTER_CHANGE_DELAY{5 * 60};
...
struct CNodeState {
    ...
    system_time m_downloading_since;
    int nBlocksInFlight;
    ...
    struct TxDownloadState {
         ...
         * time in micros -> txid)
         */
        std::multimap<mockable_time, uint256> m_tx_process_time;

        //! Store all the transactions a peer has recently announced
        std::set<uint256> m_tx_announced;

        //! Store transactions which were requested by us, with timestamp
        std::map<uint256, mockable_time> m_tx_in_flight;

        //! Periodically check for stuck getdata requests
        mockable_time m_check_expiry_timer{mockable_clock::epoch};
    };
        ...
        m_is_manual_connection (is_manual)
    {
        ...
        nStallingSince = 0;
        m_downloading_since = system_clock::epoch;
        nBlocksInFlight = 0;
...
limitedmap<uint256, mockable_time> g_already_asked_for GUARDED_BY(cs_main)(MAX_INV_SZ);
...
static bool MarkBlockAsReceived(const uint256& hash) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    if (itInFlight != mapBlocksInFlight.end()) {
        ...
        if (state->vBlocksInFlight.begin() == itInFlight->second.second) {
            ...
            state->m_downloading_since = std::max(state->m_downloading_since, system_clock::now());
        }
...
static bool MarkBlockAsInFlight(NodeId nodeid, const uint256& hash, const CBlockIndex* pindex = nullptr, std::list<QueuedBlock>::iterator** pit = nullptr) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    if (state->nBlocksInFlight == 1) {
        ...
        state->m_downloading_since = system_clock::now();
    }
...
mockable_time GetTxRequestTime(const uint256& txid) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...
void UpdateTxRequestTime(const uint256& txid, mockable_time request_time) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    auto it = g_already_asked_for.find(txid);
    if (it == g_already_asked_for.end()) {
...
mockable_time CalculateTxGetDataTime(const uint256& txid, mockable_time current_time, bool use_inbound_delay) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    mockable_time process_time;
    const mockable_time last_request_time = GetTxRequestTime(txid);
    // First time requesting this tx
    if (last_request_time == mockable_clock::epoch) {
...
void RequestTx(CNodeState* state, const uint256& txid, mockable_time current_time) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...
bool AddOrphanTx(const CTransactionRef& tx, NodeId peer) EXCLUSIVE_LOCKS_REQUIRED(g_cs_orphans)
{
    ...
    auto ret = mapOrphanTransactions.emplace(hash, COrphanTx{tx, peer, mockable_clock::now() + ORPHAN_TX_EXPIRE_TIME, g_orphan_list.size()});
    assert(ret.second);
...
unsigned int LimitOrphanTxSize(unsigned int nMaxOrphans)
{
    ...
    static mockable_time g_next_sweep GUARDED_BY(g_cs_orphans);
    mockable_time nNow = mockable_clock::now();
    if (g_next_sweep <= nNow) {
        // Sweep out expired orphan pool entries:
        int nErased = 0;
        mockable_time nMinExpTime = nNow + ORPHAN_TX_EXPIRE_TIME - ORPHAN_TX_EXPIRE_INTERVAL;
        std::map<uint256, COrphanTx>::iterator iter = mapOrphanTransactions.begin();
        while (iter != mapOrphanTransactions.end())
        {
        ...
        g_next_sweep = nMinExpTime + ORPHAN_TX_EXPIRE_INTERVAL;
    ...
    if (pfrom->m_tx_relay != nullptr) {
        ...
        while (it != pfrom->vRecvGetData.end() && (it->type == MSG_TX || it->type == MSG_WITNESS_TX)) {
            ...
            if (mi != mapRelay.end()) {
                ...
                push = true;
            } else if (mockable_clock::epoch == pfrom->m_tx_relay->m_last_mempool_req.load()) {
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    if (strCommand == NetMsgType::INV) {
        ...
        uint32_t nFetchFlags = GetFetchFlags(pfrom);
        const mockable_time current_time = mockable_clock::now();
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        std::vector<CInv> vInv;
        {
            ...
            if (pto->m_tx_relay != nullptr) {
                ...
                if (fSendTrickle && pto->m_tx_relay->fSendMempool) {
                    ...
                    pto->m_tx_relay->m_last_mempool_req = mockable_clock::now();
                }
        ...
        if (state.vBlocksInFlight.size() > 0) {
            ...
            if (system_time{std::chrono::microseconds{nNow}} > state.m_downloading_since + std::chrono::microseconds{consensusParams.nPowTargetSpacing * (BLOCK_DOWNLOAD_TIMEOUT_BASE + BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads)}) {
        ...
        if (pto->m_tx_relay != nullptr && pto->nVersion >= FEEFILTER_VERSION && gArgs.GetBoolArg("-feefilter", DEFAULT_FEEFILTER) &&
            ...
            system_time timeNow = system_clock::now();
            ...
            // until scheduled broadcast, then move the broadcast to within MAX_FEEFILTER_CHANGE_DELAY.
                ...
                pto->m_tx_relay->nextSendTimeFeeFilter = timeNow + std::chrono::seconds{GetRandInt(MAX_FEEFILTER_CHANGE_DELAY.count())};
            }
        }
    }
    return true;
