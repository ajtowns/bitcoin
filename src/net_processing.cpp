 ...
 * previous request
 */
void ConditionalBackdateTxRequestTime(const uint256& txid, std::chrono::microseconds old_request_time, std::chrono::microseconds current_time) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    auto it = g_already_asked_for.find(txid);
    if (it != g_already_asked_for.end() && it->second == old_request_time) {
        g_already_asked_for.update(it, current_time - GETDATA_TX_INTERVAL);
    }
}
...

{
    ...
    if (last_request_time.count() == 0) {
        ...
        const std::chrono::microseconds early_process_time = PoissonNextSend(current_time, std::chrono::seconds(20));
        ...
        if (vInv.size() <= MAX_PEER_TX_IN_FLIGHT + MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            ...
            for (CInv &inv : vInv) {
                if (inv.type == MSG_TX || inv.type == MSG_WITNESS_TX) {
                    ...
                    ConditionalBackdateTxRequestTime(inv.hash, in_flight_it->second, current_time);
