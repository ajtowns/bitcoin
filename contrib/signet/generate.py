...
SIGNET_HEADER_SCRIPTSIG = b"\xec\xc7\xda\xa2"
SIGNET_HEADER_WITNESS = b"\xec\xc7\xda\xa3"
...
def signet_tx(block, use_witness):
    # assumes signet solution has not been added yet so does not need
    # to be removed
    header = SIGNET_HEADER_WITNESS if use_witness else SIGNET_HEADER_SCRIPTSIG

    txs = block.vtx[:]
    txs[0] = CTransaction(txs[0])
    txs[0].vout[-1].scriptPubKey += pushdata(header)
    ## sys.stderr.write("mod-cb: %s\n" % (ToHex(txs[0])))
    hashes = []
    for tx in txs:
        tx.rehash()
        hashes.append(ser_uint256(tx.sha256))
    ...
    spk = b"\x20" + ser_uint256(mroot)
    stx = CTransaction()
    stx.vin = [CTxIn(COutPoint(block.hashPrevBlock, 0), b"", 0)]
    stx.vout = [CTxOut(block.nVersion, spk)]
    stx.nLockTime = block.nTime
    return stx

def main():
    tmpl = json.load(sys.stdin)

    cbtx = create_coinbase(height=tmpl["height"], value=tmpl["coinbasevalue"], spk=bytes())
    ...
    if signet_spk.startswith("0") or signet_spk.startswith("10"):
        use_witness = True
    else:
        use_witness = False
    ...
    cp = subprocess.run(["./bitcoin-cli","-signet",
                         ...
                         "[{\"txid\": \"%064x\", \"vout\": %d, \"scriptPubKey\": \"%s\", \"amount\": %s}]" % (block.hashPrevBlock, 0, signet_spk, MAX_MONEY)],
