...
The created database contains a table `utxos` with one of the following
schemas, depending on the set mode (see parameter '-m'/'--mode'):
   "hex": (txid TEXT, vout INT, value INT, coinbase INT, height INT, scriptpubkey TEXT)
 "bytes": (txid BLOB, vout INT, value INT, coinbase INT, height INT, scriptpubkey BLOB)
"""
import argparse
import os
import sqlite3
...
def main():
    ...
    parser.add_argument('-m', '--mode', default='hex',
                        help='format of txids and scriptpubkeys (default: "hex", values: "hex", "bytes")')
    ...
    if args.mode not in ("hex", "bytes"):
        print(f"Error: provided mode '{args.mode}' is invalid (must be 'hex' or 'bytes')")
        sys.exit(1)
    store_blobs = args.mode == "bytes"
    ...
    for coin_idx in range(1, num_utxos+1):
        ...
        if store_blobs:
            ...
            write_batch.append((prevout_hash[::-1].hex(), prevout_index, amount, is_coinbase, height, scriptpubkey.hex()))
